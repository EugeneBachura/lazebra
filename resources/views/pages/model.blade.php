...
@section('content')
    ...
    <div class="cars-list">
        @foreach ($model_ages as $model_age)

        @if (($model_age->url != null)&&($covers == 'model'))
            <a href="{{ $base_url }}{{ $model_age->url }}/" class="cars-list__item">
                <div class="cars-list__bg" style="background-image: url('/{{$model_age->img}}');"></div>
                <h3 class="cars-list__name">{{--$name_brand--}}{{$model_age->name}}</h3>
            </a>
        @endif

        @if (($model_age->url != null)&&($covers == 'frame'))
            <a href="{{ $base_url }}{{ $model_age->url }}" class="cars-list__item">
                <div class="cars-list__bg" style="background-image: url('/{{$model_age->img}}');"></div>
                <h3 class="cars-list__name">{{$name_brand}} {{$model_age->name}}</h3>
            </a>
